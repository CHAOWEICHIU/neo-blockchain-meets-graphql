...
const ExchangeType = require('./exchange')
 ...
 */
const fetchCmcListingIdsBySymbols = symbols => request
  .get('https://api.coinmarketcap.com/v2/listings/')
  .then(({ body: { data } }) => symbols.map((x) => {
    const result = _.find(data, x)
 ...
 */
const fetchCmcDataBySymbols = symbols => fetchCmcListingIdsBySymbols(symbols)
  .then(resultSymbols => Promise.all(
    resultSymbols.map(symbol => (symbol.id
      ? request.get(`https://api.coinmarketcap.com/v2/ticker/${symbol.id}/`)
      : { body: { data: null } })),
  )
    .then(data => data.map(d => d.body.data))
    .then(data => symbols.map((symbol, symbolIndex) => Object.assign(
      {},
      symbol,
...
const getNativeAssetsFunction = (address, net) => Neon
  .api
  .default
  .get.balance(
    net,
...
const getNep5TokensThatHasBalance = address => new Promise((resolve) => {
  request
    .get('https://raw.githubusercontent.com/CityOfZion/neo-tokens/master/tokenList.json')
    .then(res => _.flatMap(_.toPairsIn(JSON.parse(res.text)))
      .filter(x => _.isObject(x))
      .map(x => x.networks['1'].hash))
    .then(scriptHashes => Promise.all(
      scriptHashes.map(scriptHash => Neon.api.nep5.getTokenInfo('http://seed7.bridgeprotocol.io:10332', scriptHash)),
    )
      .then(
        nep5Tokens => nep5Tokens
          .map((nep5Token, nep5TokenIndex) => Object.assign(
            {},
            nep5Token,
            { scriptHash: scriptHashes[nep5TokenIndex] },
          )),
      )
      .then(nep5Tokens => Promise.all(
        nep5Tokens.map(nep5Token => Neon
          .api
          .nep5
          .getTokenBalance('http://seed7.bridgeprotocol.io:10332', nep5Token.scriptHash, address)
          .then(balance => Object.assign(
            {},
            nep5Token,
            { balance },
          ))),
...
const AssetNameAmountType = new GraphQLObjectType({
  ...
  fields: {
    ...
    assetSummary: {
      ...
      resolve: parent => fetchCmcDataBySymbols([{ symbol: parent.name }])
...
const Nep5AssetType = new GraphQLObjectType({
  ...
  fields: {
    ...
    assetSummary: {
      type: AssetSummaryType,
      resolve: parent => fetchCmcDataBySymbols([{ symbol: parent.symbol }])
        .then(data => ({ cmcUsdPrice: data[0].USD })),
    },
  },
})
...
const AssetType = new GraphQLObjectType({
  ...
  fields: {
    nativeAssets: {
      ...
      resolve: ({ addresses, net }) => Promise.all(
        addresses.map(address => getNativeAssetsFunction(address, net)),
      )
        .then(data => _.flattenDeep(data)
          .reduce((container, item) => container.map(x => (x.name === item.name
            ? Object.assign({}, x, { balance: _.toNumber(x.balance) + _.toNumber(item.balance) })
            : x)), [{ name: 'GAS', balance: 0 }, { name: 'NEO', balance: 0 }]))
      ,
    },
    nep5Assets: {
      type: GraphQLList(Nep5AssetType),
      resolve: parent => Promise.all(
        parent.addresses.map(address => getNep5TokensThatHasBalance(address)),
      )
        .then(data => _.flatMapDeep(data)
          .reduce((container, item) => {
            const targetIndex = _.findIndex(container, x => x.name === item.name)
            if (targetIndex !== -1) {
              container[targetIndex].balance = container[targetIndex].balance + item.balance // eslint-disable-line
            } else {
              container.push(item)
            }
            return container
          }, []))
        .then(nep5Tokens => nep5Tokens.map(nep5Token => Object.assign(
          {},
          nep5Token,
          { tokenInformation: nep5Token },
        ))),
    },
  },
})
...
const RootQuery = new GraphQLObjectType({
  ...
  fields: {
    exchange: {
      type: ExchangeType,
      args: {
        cobinhoodApiKey: { type: GraphQLNonNull(GraphQLString) },
        binanceApiKey: { type: GraphQLNonNull(GraphQLString) },
      },
